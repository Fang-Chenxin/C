...
更新逗留时间 删除第一队列第一个业务（已完成）*/
 ...
 if(back(eq)->beginTime<closeTime)
  ...
  state = 0;
  exitTime = currentTime + (back(eq)->endTime - back(eq)->beginTime);
}
 ...
 if(front(fq)->endTime<closeTime)
  ...
  back(eq)->endTime = (front(fq)->beginTime + dealTime);
   ...
   back(eq)->endTime = (judgetime + dealTime);
    ...
    back(eq)->beginTime = front(fq)->beginTime;
...
{ /*在对列中寻找可处理元素，对其进行处理，
 ...
 更新逗留时间(已完成) */
  ...
  { // search如果没有返回0，有符合的返回要处理的人在sq的第几个
    ...
    printf("请输入序号为%d的客户的取钱时间:\t", indexn);

    scanf("%d", &dealTime);


    back(eq)->endTime = dealTime + randomTemp;
...
//          currentTime += arriveTime;
      ...
      if (1 == state && NULL != fq.head && exitTime <= closeTime)
