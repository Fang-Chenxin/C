...
{       
    ...
    default:  return(-1);  /*当输入参数不是四则运算、括号、等号时，返回－1*/        
...
{  
    ...
    DS->Dbase = (double *)malloc(STACK_INIT_SIZE*sizeof(double));
    if(!DS->Dbase) return 0;
    DS->Dtop = DS->Dbase;
...
{       
    ...
    {
        printf("OPND:%d,%d,%d\n",DS->Dtop-DS->Dbase,DS->Dstacksize,DS->Dtop-DS->Dbase >= DS->Dstacksize);
        DS->Dbase = (double *)realloc(DS->Dbase,(STACK_INIT_SIZE+STACK_INCREMENT)*sizeof(double));
        if (!DS->Dbase) return 0;
...
if(DS->Dbase == NULL) return 0;
if(DS->Dbase == DS->Dtop) return 0;
*e=*--DS->Dtop;
return 1;
...
{       
    ...
    if(DS->Dbase == NULL) return DBL_MAX;
    if(DS->Dbase == DS->Dtop) return DBL_MAX;
    return *(DS->Dtop-1);
...
* 处理规则及返回值：释放栈底指针所指空间，并将数据栈的栈底和栈顶指针置空，数据栈Ostacksize置为0。最后返回1*/       
...
* 处理规则及返回值：从内存堆中申请STACK_INIT_SIZE个整型空间，将栈顶指针与栈底指针都指向初始地址，给Ostacksize赋值STACK_INIT_SIZE   
...
{       
    OS->Obase = (char *)malloc(STACK_INIT_SIZE*sizeof(char));
    if(!OS->Obase) return 0;
    OS->Otop = OS->Obase;
...
if ((OS->Otop-OS->Obase) >= OS->Ostacksize) 
    {
    printf("OPTR:%d,%d,%d\n",OS->Otop-OS->Obase,OS->Ostacksize,OS->Otop-OS->Obase >= OS->Ostacksize);
    OS->Obase = (char *)realloc(OS->Obase,(STACK_INIT_SIZE+STACK_INCREMENT)*sizeof(char));
    if (!OS->Obase) return 0;
    OS->Otop = OS->Obase + OS->Ostacksize;
    OS->Ostacksize += STACK_INCREMENT;
    }
    *OS->Otop++ = e;
    return 1;
...
if(OS->Obase == NULL) return 0;
if(OS->Obase == OS->Otop) return 0;
*e=*--OS->Otop;
return 1;
...
{       
    ...
    if(OS->Obase == NULL) return DBL_MAX;
    if(OS->Obase == OS->Otop) return DBL_MAX;
    return *(OS->Otop-1);
...
* 处理规则及返回值：释放栈底指针所指空间，并将符号栈的栈底和栈顶指针置空，符号栈Ostacksize置为0。最后返回1*/       
