...
typedef struct QUEUE_NODE  //节点队列
...
{ 
    ...
    QUEUE_NODE_PTR newNode = (QUEUE_NODE_PTR)malloc(sizeof(QUEUE_NODE));
    newNode->data = MRN;
    newNode->next = NULL;
    q->rear->next = newNode;
    q->rear = newNode;
...
if (q->front == q->rear || q->Len == 0) return 1;
return 0;
...
1.函数接口定义为：int QTraverse(LINK_QUEUE *q)
