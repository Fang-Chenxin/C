...
void SpeedPlan_S(double s_target, double v_target, double a_max, double j_max);   // 速度规划函数
double calculate_s_T(double v_act, double a_max, double j_max);                   // 计算匀速阶段的位移
double lookUp_t(double xx, double j_max);                                         // 计算距离为xx时的时间
void Init(double j_max);                                                          // 初始化函数

double j(double tt, double j_max);                                   // 计算加加速度
double a(double tt, double j_max);                                   // 计算加速度
double v(double tt, double j_max);                                   // 计算速度
double s(double tt, double j_max);                                   // 计算位移
double Anwser_Equation(double xx, double a, double b, double j_max); // 计算二次方程的解
...
void SpeedPlan_S(double s_target, double v_target, double a_max, double j_max)
{
    ...
    s_T = calculate_s_T(v_act, a_max, j_max);
    ...
    {
        // 重新规划最大速度v_max,假设此时不包含匀加速和匀减速，恰好达到a_max
        s_T = calculate_s_T(v_act, a_max, j_max);
...
double calculate_s_T(double v_act, double a_max, double j_max)
{
    ...
    vv[0] = 0.5 * a_act * (t[0] - sin(w * t[0]) / w); // 加加速结束后的速度
    vv[1] = a_act * t[1] + vv[0];                     // 匀加速结束后的速度

    x[0] = 0.25 * a_act * (t[0] * t[0] + 2 * cos(w * t[0]) / (w * w)) - a_act / (2 * w * w);                       // 加加速
    x[1] = 0.5 * a_act * t[1] * t[1] + vv[0] * t[1] + x[0];                                                        // 匀加速
    x[2] = 0.25 * a_act * (t[2] * t[2] - 2 * cos(w * t[2]) / (w * w)) + vv[1] * t[2] + a_act / (2 * w * w) + x[1]; // 减加速

    return x[2] * 2; // 加速与减速段总位移
}
